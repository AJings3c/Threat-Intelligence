...
export function buildArchitectureThreatModel(
  ...
  return {
    ...
    assets: [
      ...
      {
        id: 'store',
        name: text(zh, 'Aggregator store and durable SQLite intelligence history', '聚合存储与 SQLite 持久化情报历史'),
        kind: 'data_store',
        trustZone: text(zh, 'data', '数据区'),
        criticality: 'high',
        data: list(
          zh,
          ['IOC/CVE snapshots', 'source observations', 'durable jobs', 'cases/rules', 'audit events'],
          ['IOC/CVE 快照', '来源观测', '持久化任务', '案件/规则', '审计事件'],
        ),
    ...
    dataFlows: [
      ...
      },
      {
        ...
        crossesTrustBoundary: true,
        data: list(zh, ['refresh timestamps', 'aggregate counts'], ['刷新时间戳', '聚合计数']),
        trustBoundary: 'browser-api',
        threatSurface: list(zh, ['long-lived connection', 'authenticated polling fallback'], ['长连接', '认证场景轮询回退']),
      },
      {
        id: 'feed-flow',
        from: 'feeds',
      ...
      },
      {
        ...
        protocol: 'Local MMDB or opt-in HTTPS API',
        crossesTrustBoundary: true,
        data: list(zh, ['public IP indicators', 'country', 'latitude/longitude'], ['公开 IP 指标', '国家', '经纬度']),
        trustBoundary: 'api-internet',
        threatSurface: list(zh, ['third-party disclosure when explicitly configured', 'third-party availability'], ['显式配置第三方服务时的指标披露', '第三方可用性']),
      },
      {
        id: 'persist-flow',
        from: 'api',
    ...
    threatMatrix: [
      {
        ...
        stride: {
          Spoofing: [text(zh, 'Stolen API token or forged proxy identity impersonates analyst/admin traffic', '被盗 API Token 或伪造代理身份可冒充分析员/管理员请求')],
    ...
    scenarios: [
      ...
      scenario(
        ...
        list(
          ...
          [
            ...
            'Verify Content-Digest on every export and configure HMAC signing for regulated downstream exchange',
          ],
          ['仅通过受信入口和 TLS 提供导出', '记录报告生成和配置测试的审计事件', '验证所有导出的 Content-Digest，并在合规交换中配置 HMAC 签名'],
        ),
        {
          ...
          dread: dread({
            ...
            rationale: list(
              zh,
              ['Exports are trusted by downstream consumers', 'Exports include a SHA-256 digest and optionally an HMAC signature'],
              ['导出内容会被下游消费者信任', '导出已包含 SHA-256 摘要，并可选附带 HMAC 签名'],
            ),
          }),
          ...
          verification: list(
            zh,
            ['Export Markdown and validate generated timestamp/provenance', 'Verify digest and HMAC test vectors against the exact body'],
            ['导出 Markdown 并校验生成时间/来源信息', '根据响应原文验证摘要和 HMAC 测试向量'],
          ),
        },
      ),
      scenario(
      ...
      scenario(
        ...
        list(
          ...
          [
            ...
            'Tune provider cache TTL and circuit-breaker thresholds for production traffic',
            'Restrict enrichment and config testing to analyst/admin roles',
          ],
          ['保持 API 路由限流开启', '根据生产流量调整服务商缓存 TTL 和熔断阈值', '将富化和配置测试限制为分析员/管理员角色'],
        ),
        {
          ...
          dread: dread({
            ...
            rationale: list(
              ...
              [
                ...
                'Successful results are cached for one hour and failures are isolated by provider circuit breakers',
              ],
              [`内存 API 限流约为每窗口 ${rateLimitMax} 次请求`, '成功结果缓存一小时，失败由服务商级熔断器隔离'],
            ),
          }),
          controls: ['api-rate-limit', 'role-scoped-auth', 'provider-backoff'],
          treatment: 'implemented',
          verification: list(zh, ['Exercise rate-limit and provider-circuit tests', 'Tune thresholds before broad deployment'], ['执行限流和服务商熔断测试', '大规模部署前调优阈值']),
        },
      ),
      scenario(
        ...
        authConfigured
          ...
          : text(zh, 'Development open-auth mode can expose analyst/admin privileges when deployed incorrectly', '开发环境开放鉴权模式若被错误部署，可能暴露分析员/管理员权限'),
        'Elevation of Privilege',
        'high',
        70,
        [
          authConfigured
            ? text(zh, 'API token, role-scoped token, or trusted authentication proxy is configured', '已配置 API Token、角色范围 Token 或可信认证代理')
            : text(zh, 'Production fails closed without authentication; development permits local open-admin mode', '生产环境未配置鉴权时拒绝访问；开发环境允许本地开放管理员模式'),
        ...
        ),
        {
          ...
          dread: dread({
            ...
            reproducibility: 8,
            exploitability: authConfigured ? 5 : 6,
            affectedUsers: 7,
            discoverability: 8,
            rationale: list(
              zh,
              [
                ...
                authConfigured ? 'Deployment has an auth signal configured' : 'Only non-production or an explicit unsafe override permits open-admin access',
              ],
              ['代码中已实现路由级角色', authConfigured ? '部署已有鉴权信号' : '只有非生产环境或显式不安全开关才允许开放管理员访问'],
            ),
          }),
      ...
      scenario(
        ...
        text(zh, 'Optional third-party geolocation can disclose tracked indicators', '可选第三方地理定位可能披露被跟踪指标'),
        'Information Disclosure',
        'medium',
        65,
        list(
          zh,
          [
            'Online geolocation is disabled unless an HTTPS GEO_LOOKUP_URL template is configured',
            'Offline MaxMind lookup keeps indicator values local',
          ],
          ['仅在配置 HTTPS GEO_LOOKUP_URL 模板后启用在线地理定位', '离线 MaxMind 查询不会外发指标值'],
        ),
        list(
          zh,
          [
            'Prefer a local MaxMind database for production use',
        ...
        ),
        {
          ...
          threat: text(zh, 'A configured geolocation provider observes tracked IP indicators or returns incorrect coordinates.', '已配置的地理服务商观察被跟踪 IP，或返回错误坐标。'),
          ...
          dread: dread({
            ...
            affectedUsers: 3,
            discoverability: 8,
            rationale: list(zh, ['Online lookup is HTTPS-only and opt-in', 'Geolocation is contextual and not a blocking decision by itself'], ['在线查询仅支持 HTTPS 且默认关闭', '地理定位本身只是上下文，不直接作为阻断决策']),
          }),
          controls: ['local-geoip'],
          treatment: 'implemented',
          verification: list(zh, ['Assert no HTTP geolocation URL remains and prefer GEOLITE2_DB in production'], ['确认不存在 HTTP 地理定位 URL，并在生产优先使用 GEOLITE2_DB']),
        },
      ),
      scenario(
    ...
    attackPaths: [
      ...
      {
        id: 'ap-third-party-geo-context',
        actor: text(zh, 'Configured geolocation provider or compromised provider', '已配置或被攻陷的地理定位服务商'),
        objective: text(zh, 'Observe tracked IPs or manipulate geographic context', '观察被跟踪 IP 或操纵地理上下文'),
        entryPoint: 'backend/src/geo.ts optional HTTPS lookup',
        path: list(
          zh,
          ['Receive an opt-in IP lookup', 'Record or alter IP-to-location response', 'Dashboard map renders misleading context'],
          ['接收显式启用的 IP 查询', '记录或修改 IP 到位置响应', '仪表盘地图展示误导性上下文'],
        ),
    ...
    controls: [
      ...
      {
        id: 'provider-backoff',
        name: text(zh, 'Provider-specific enrichment cache and circuit breaker', '服务商级富化缓存和熔断'),
        status: 'implemented',
        owner: text(zh, 'Backend', '后端'),
        scenarios: ['enrichment-abuse'],
        verification: [text(zh, 'Tests verify circuit isolation; only successful enrichment results are cached within TTL', '测试验证熔断隔离，且只有成功富化结果会在 TTL 内缓存')],
      },
      {
        id: 'signed-exports',
        ...
        verification: [text(zh, 'Verify Content-Digest and, when configured, X-Content-HMAC-SHA256 against the exact response body', '根据响应原文验证 Content-Digest；配置签名时同时验证 X-Content-HMAC-SHA256')],
      },
      {
        id: 'local-geoip',
        ...
        status: 'implemented',
        owner: text(zh, 'Backend / DevOps', '后端 / DevOps'),
        scenarios: ['third-party-geo-enrichment'],
    ...
    nextSteps: list(
      ...
      [
        ...
        'Prefer local GeoIP when map context is used in incident workflow',
        'Monitor and tune enrichment cache/circuit settings before heavy analyst usage',
      ...
      [
        ...
        '当地图上下文进入事件流程时优先使用本地 GeoIP',
        '高频分析员使用前监控并调优富化缓存/熔断参数',
